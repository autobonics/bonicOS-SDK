...
class PreciseMotionController(ControllerBase):
    ...
    def run_queue(self, block: bool = True) -> bool:
        ...
        try:
            ...
            # threaded WASM build — see motion.py's MotionController for the
            # same constraint). Run the drain inline instead of backgrounding
            # it; `block=False` degrades to synchronous here since there's no
            # thread left to poll for later.
