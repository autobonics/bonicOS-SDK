...
def test_drive_distance_reaches_target(robot, transport) -> None:
    transport.set_telemetry("odom", {"x": 0.0, "y": 0.0, "theta": 0.0, "vx": 0, "vtheta": 0})

    def updater() -> None:
        time.sleep(0.05)
        transport.set_telemetry("odom", {"x": 1.0, "y": 0.0, "theta": 0.0, "vx": 0, "vtheta": 0})
...
def test_drive_distance_without_odom_fails_fast(robot, transport) -> None:
    ...
    assert time.monotonic() - start < 1.0  # bounded by `timeout`, not ODOM_WAIT_TIMEOUT_S
...
def test_rotate_angle_accumulates_signed_delta(robot, transport) -> None:
    ...
    def updater() -> None:
        ...
        transport.set_telemetry(
            "odom", {"x": 0.0, "y": 0.0, "theta": math.radians(90), "vx": 0, "vtheta": 0}
        )
...
def test_draw_square_composes_four_legs(robot, transport) -> None:
    with patch.object(robot.precise, "drive_distance", return_value=True) as drive, patch.object(
        robot.precise, "rotate_angle", return_value=True
    ) as rotate:
...
def test_draw_square_stops_on_first_failure(robot, transport) -> None:
    with patch.object(robot.precise, "drive_distance", return_value=False) as drive, patch.object(
        robot.precise, "rotate_angle", return_value=True
    ) as rotate:
